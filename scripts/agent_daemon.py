...
Usage:
    ...
    python scripts/agent_daemon.py --role research_director
    ...
    python scripts/agent_daemon.py --unattended       # execute in-scope work without approval
    ...
    try:
        ...
        for msg in messages:
            if not getattr(msg, "authorized", False):
                emit_damage(
                    source="agent_daemon.telegram",
                    kind="unauthorized_telegram_message",
                    detail=f"Rejected unauthorized telegram message command={getattr(msg, 'command', 'unknown')}",
                    severity="warn",
                )
...
def discover_work(role_id: str = "manager") -> list[Candidate]:
...
- If you author new Python, make it a reusable mechanized primitive in
  `scripts/` or `src/`; do not hide one-off logic in an unrelated shell
  wrapper.
...
def _find_existing_pending_gate(role_id: str, candidate: Candidate) -> Optional[str]:
    ...
    reuse that gate_id instead of writing a new one. Telegram won't refire either.
    """
...
def _write_proposal_gate(
    *,
    ...
    it (returns is_fresh=False) to prevent duplicate Telegram proposals on
...
def _resolve_gate_from_telegram(gate_id: str, role_id: str) -> None:
    """Convert authorized Telegram commands into the same resolved gate.
    ...
    chars of the originating gate_id (per `send_telegram` in this file).
...
def tick(
    ...
    # 2. Discover work
    candidates = discover_work(role_id=role_id)
    if not candidates:
    ...
    if unattended and not approval_required:
        ...
        if not was_fresh:
            ...
            # Don't re-fire telegram for a reused gate; principal already saw it
        else:
            ...
            if telegram_available():
                ...
                send_telegram(friendly, priority="high", gate_id=gate_id)
            else:
                log.warning("No telegram configured. Waiting for Orbit/filesystem gate resolution.")
        ...
        if chosen != "approve":
            ...
            if chosen == "stop":
                send_telegram("Daemon stopped by resolved gate.")
                sys.exit(0)
            return False
        append_transition(
...
def main():
    ...
    if args.variable_interval:
        log.info(
            ...
            f"{args.idle_interval}s idle). Send STOP via telegram or dashboard to halt."
        )
        if telegram_available():
            send_telegram(
