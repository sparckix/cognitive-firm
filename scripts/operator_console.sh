...
usage() {
  cat <<'EOF'
Usage:
  scripts/operator_console.sh claude
  scripts/operator_console.sh codex
  scripts/operator_console.sh docker-claude

Purpose:
  Open a principal-operated interactive agent console in this repo without
  starting the role daemon, work discovery, gates, or the validator loop.

Modes:
  claude         Run local Claude Code CLI interactively in the repo.
  codex          Run local Codex CLI interactively in the repo.
  docker-claude  Run Claude Code inside the operator-console Docker profile.

This is the Mode A operator-agent path. It is for direct collaboration, like
the current human <-> Codex/Claude session. If the work changes durable repo
state, the agent is still expected to follow AGENTS.md closure discipline.
EOF
}
