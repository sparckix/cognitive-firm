...
def run_replay(root: Path) -> dict[str, Any]:
    ...
    unsafe_report = evaluate_offline_policy_candidate(
        summary.records,
        candidate_policy_id="policy.support.renewals-auto-send",
    ...
    append_policy_evaluation(unsafe_report, log_path=logs["evaluations"])

    safe_packet = build_policy_promotion_packet(
        safe_report,
    ...
    unsafe_packet = build_policy_promotion_packet(
        unsafe_report,
        proposed_by="role.support_manager",
        authority_diff_ref="authority-diff://renewals-auto-send",
        title="Review renewals auto-send routing policy",
    )
    append_policy_promotion_packet(safe_packet, log_path=logs["packets"])
    append_policy_promotion_packet(unsafe_packet, log_path=logs["packets"])

    replayed_reports = list_policy_evaluations(log_path=logs["evaluations"])
    replayed_packets = list_policy_promotion_packets(log_path=logs["packets"])
    report_by_id = {report.evaluation_id: report for report in replayed_reports}
    packet_rows = []
