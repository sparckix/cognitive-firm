...
Usage:
  python scripts/dispatch_external_prover.py \\
  ...
  echo "Is X a faithful encoding of Y? Specify divergences." | \\
    python scripts/dispatch_external_prover.py --substrate meta-architecture

Output:
  * Verdict + reasoning printed to stdout.
  * One row appended to analytics/external_prover_ledger.jsonl
  * One row appended to analytics/pattern_deployment_ledger.jsonl
...
REPO = _resolve_repo()
EXTERNAL_LEDGER = REPO / "analytics/external_prover_ledger.jsonl"
PATTERN_LEDGER = REPO / "analytics/pattern_deployment_ledger.jsonl"
...
def cumulative_session_spend(rows: list[dict]) -> float:
    return sum(r.get("estimated_cost_usd", 0.0) for r in rows)
...
def dispatch_openai(
    *,
    ...
    if use_reasoning:
        # Reasoning models: max_completion_tokens (not max_tokens) and
        # reasoning_effort. Some legacy o-series accept reasoning_effort.
...
def main() -> int:
    ...
    out_path = REPO / f"analytics/external_prover_responses/{dispatch_id}.md"
