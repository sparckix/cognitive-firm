...
def _setup(monkeypatch, tmp_path):
    ...
    from cognitive_firm.role_extensions import frontier_state as fs
...
def test_unknown_action_kind_returns_error(monkeypatch, tmp_path):
    ...
    from cognitive_firm.role_extensions.iter_action_executor import execute_action
...
def test_demote_route_missing_returns_failure(monkeypatch, tmp_path):
    _setup(monkeypatch, tmp_path)
    from cognitive_firm.role_extensions.frontier_state import load_state
    from cognitive_firm.role_extensions.iter_action_executor import execute_action
...
def test_drain_pending_executes_in_order(monkeypatch, tmp_path):
    ...
    (project_dir / "evidence.txt").write_text("# header\n")
    (project_dir / "project_charter.md").write_text("# Charter\n")
    from cognitive_firm.role_extensions.frontier_state import load_state, queue_action
    from cognitive_firm.role_extensions.iter_action_executor import drain_pending
    s = load_state("test_proj")
    queue_action(s, {
        "action_kind": "mutate_charter",
...
def test_safety_rail_blocks_excess_spend(monkeypatch, tmp_path):
    ...
    import cognitive_firm.role_extensions.iter_action_executor as iax
