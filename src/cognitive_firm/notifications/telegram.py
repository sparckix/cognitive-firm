...
class InboundMessage:
    ...
    callback_gate_id: Optional[str] = None  # set when message originated from
                                            # an inline-keyboard tap whose
                                            # callback_data was "approve:<gate_id>"
                                            # or "skip:<gate_id>" / "stop:<gate_id>".
                                            # Used by daemon to route the response
                                            # to the correct pending gate.
...
def poll_inbound(consume: bool = True, max_messages: int = 50) -> list[InboundMessage]:
    ...
    params = {
        ...
        # Explicitly request both message and callback_query update types.
        # If allowed_updates is omitted, Telegram defaults to text-only
        # (callback_query updates from inline-keyboard taps are dropped).
    ...
    for update in resp.get("result", []):
        ...
        # Two update kinds we accept: regular text messages AND callback_query
        # (inline-keyboard button taps). For callback_query we synthesize an
        # InboundMessage with raw_text=APPROVE/SKIP/STOP so the existing
        # approval flow works for both typing AND tapping.
        cbq = update.get("callback_query")
        if cbq:
            cbq_id = cbq.get("id")
            data = (cbq.get("data") or "").strip()  # e.g. "approve:gate_xyz_short"
            parts = data.split(":", 1)
            cb_action = parts[0].upper() if parts else ""
            cb_gate_id = parts[1] if len(parts) > 1 else None  # gate_id suffix
            cb_chat = (cbq.get("message", {}).get("chat", {}) or {})
            chat_id = int(cb_chat.get("id", 0))
            ts = int(cbq.get("message", {}).get("date", time.time()))
            authorized, reason = _authorize(chat_id, creds.principal_chat_id)
            # Acknowledge callback to clear the button's loading spinner.
            if cbq_id:
                try:
                    _api_call(
                        creds.bot_token, "answerCallbackQuery",
                        {"callback_query_id": cbq_id, "text": f"✓ {cb_action}"},
                        timeout=5.0,
                    )
                except Exception:  # noqa: BLE001
                    pass
            cmd, payload = _classify(cb_action)
...
def push_notification(
    ...
    inline_buttons: optional list of [label, callback_data] pairs.
    Renders as a one-row inline keyboard for tap-to-respond. Each
    callback_data ≤64 bytes per Telegram's spec. Example:
        inline_buttons=[("Approve", "approve:gate_xyz"),
                        ("Skip", "skip:gate_xyz"),
                        ("Stop", "stop:gate_xyz")]

    Returns True on success, False on any failure (network, missing
    creds, HTTP). Failures are logged but do NOT raise.
    """
    ...
    if creds is None:
        ...
        return False
    # parse_mode dropped (was "Markdown") — role ids with underscores
    # break Telegram-Markdown parsing. Plain text is robust + readable.
    params: dict = {
        "chat_id": str(creds.principal_chat_id),
        "text": full,
    }
    if inline_buttons:
        keyboard = [[
            ...
            for label, cb in inline_buttons
        ]]
