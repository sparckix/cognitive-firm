...
from src.cognitive_firm.orchestration.core import GoalConfig, GoalState, StageDefinition, StageResult
...
def stage_guidance(
    ...
    if target == "operator_manual":
        if stage.is_gate:
            return (
                ...
                f"    python -m src.cognitive_firm.orchestration.cli resume {slug}"
            )
        return (
            f"OPERATOR: {name}\n"
            f"  {desc}\n"
            f"  Action: complete the work, then advance:\n"
            f"    python -m src.cognitive_firm.orchestration.cli advance {slug}"
        )
    ...
    if target == "autoresearch":
        ...
        if project_dir and rubric:
            cmd = (
                f"python -m src.cognitive_firm.validator.autoresearch_loop "
