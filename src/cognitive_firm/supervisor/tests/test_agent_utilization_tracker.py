...
def _force_root(monkeypatch, tmp_path: Path) -> Path:
    ...
    from cognitive_firm.supervisor import agent_utilization_tracker as aut
...
def test_check_utilization_allows_session_cap(monkeypatch, tmp_path):
    _force_root(monkeypatch, tmp_path)
    from cognitive_firm.supervisor.agent_utilization_tracker import check_utilization_allows
...
def test_unknown_dimension_raises(monkeypatch, tmp_path):
    _force_root(monkeypatch, tmp_path)
    from cognitive_firm.supervisor.agent_utilization_tracker import get_utilization_pct
...
""", encoding="utf-8"
    ...
    try:
        os.chdir(tmp_path)
        from cognitive_firm.supervisor.agent_utilization_tracker import _role_caps
