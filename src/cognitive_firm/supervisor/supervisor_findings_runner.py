...
def load_reviewer_domains(domains: list[str]) -> str:
    ...
    Thin adapter over :mod:`src.cognitive_firm.personas.registry`.
    """
    if not domains:
        return ""

    from src.cognitive_firm.personas.registry import load_personas, format_many_for_injection
...
def run_findings_debate(
    *,
    ...
    if reviewer_domains is not None:
        ...
        from src.cognitive_firm.personas.routing import auto_select_from_workspace
    ...
    for cycle_index in range(1, max_cycles + 1):
        ...
        if state.status == DebateStatus.CONVERGED:
            ...
            if _route_result and _route_result.dynamic_personas:
                from src.cognitive_firm.personas.routing import promote_dynamic_persona
