...
def record_agent_session(*, role_id: str, agent_cli: str,
    ...
    # Warn-threshold detection: log + (optionally) push to principal via
    # the GP-128b Telegram channel. Push is best-effort: ImportError or
    # missing creds → fall back to log-only. The recorded entry is always
...
def _emit_warn_notification(*, role_id: str, agent_cli: str,
    ...
    Channels (in priority order):
      1. GP-128b Telegram push (`src.cognitive_firm.notifications.push_notification`)
    ...
    try:
        ...
        log.debug("agent_utilization Telegram push failed: %s", exc)
...
def record_override_event(*, kind: str,
    ...
    each):
      ...
      3. (best-effort) Telegram push notifying the principal of their
         own override — the friction-of-being-watched defense
    ...
    try:
        ...
        log.debug("override_event Telegram push failed: %s", exc)
