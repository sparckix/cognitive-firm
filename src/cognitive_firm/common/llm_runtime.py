...
def _bootstrap_dotenv_if_needed() -> None:
    ...
    No-op if all 3 keys already present in env (the local developer flow where
    keys are exported in shell). Reads .env quietly; no override of present env.
    """
    if all(os.environ.get(k) for k in ("ANTHROPIC_API_KEY", "OPENAI_API_KEY")):
        return  # at least the two primary keys are set; no bootstrap needed
    try:
